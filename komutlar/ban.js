...
if (!bannedmember) return message.channel.send(new Discord.MessageEmbed().setColor("RED").setAuthor(`Mübarek Peymacun `, message.author.avatarURL()).setDescription(`**⚠️ • Bir Kişi Etiketlemelisin**`))
  ...
  if (!sebep) return message.channel.send(new Discord.MessageEmbed().setColor("RED").setAuthor(`Mübarek Peymacun `, message.author.avatarURL()).setDescription(`**⚠️ • Sebep Belirtmelisin**`))
     message.channel.send(new Discord.MessageEmbed().setColor("RED").setAuthor(`Mübarek Peymacun`, message.author.avatarURL()).setDescription(`**⚠️ • Kullanıcıyı Banlamak İstediğinizden Emin Misiniz ? Lütfen __evet (e)__ veya __hayır (h)__ İle Cevap Verin.\n\n\`30\` Saniye İçerisinde İptal Edilcektir**`))
  let uwu = false; 
  while (!uwu) {
